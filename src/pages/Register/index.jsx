...
const { data, error: err } = await authService.signInWithGoogle();
      if (err) {
        if (import.meta.env.DEV) console.error('[Register] Google signIn error:', err);
      ...
      if (import.meta.env.DEV) console.error('[Register] Google signIn unexpected error:', e);
      setGoogleLoading(false);
...
if (err) {
      // Log lỗi thật trong development để dễ debug (chỉ khi DEV).
...
const { error: err } = await authService.resendConfirmation({ email: email.trim() });
    setResending(false);
    if (err) {
      if (import.meta.env.DEV) console.error('[Register] resendConfirmation error:', err);
      setError(getAuthErrorMessage(err));
      return;
    }
    ...
    <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center px-4 py-12">
      <div className="w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-8 shadow-sm">
        ...
        <div className="mt-6">
          ...
          >
            <GoogleIcon />
            {googleLoading ? 'Đang kết nối với Google...' : 'Đăng ký với Google'}
          </button>
        </div>
